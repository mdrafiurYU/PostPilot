// Database access for the Compression Engine

import { Pool } from 'pg'
import type { Rendition, Asset, AssetStatus, Adaptation } from '@postpilot/types'
...
export async function insertRendition(rendition: Rendition): Promise<Rendition> {
  const result = await pool.query<Rendition>(
    `INSERT INTO renditions
      (id, asset_id, codec, resolution, width, height, bitrate_kbps, vmaf_score, file_size_bytes, s3_key, created_at)
...
export async function getRenditionsByAssetId(assetId: string): Promise<Rendition[]> {
  const result = await pool.query<Rendition>(
    'SELECT * FROM renditions WHERE asset_id = $1 ORDER BY bitrate_kbps ASC',
    [assetId],
  )
  return result.rows
}
